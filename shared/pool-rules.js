...
/** 买券 tx → 打款池 */
function buildEntries(poolId, txs) {
...
function applyLifecycle(entries) {
...
function applyPayVerifications(entries, exitPoolTxs, exitPoolAddress, evaluationMs) {
  ...
  if (!pendingAssigns.length) return { entries, verified: [], expired: [] };

  const { verified, expired } = derivePayVerifications(
    pendingAssigns,
    exitPoolTxs,
    exitPoolAddress,
    evaluationMs,
  ...
  return { entries, verified, expired };
}
...
function runDayMatch(
  ...
  let entries = mergeEntryStates(buildEntries(poolId, dayPurchase), stateMap);
  entries = applyLifecycle(entries);

  const cfg = poolConfig(poolId);
  const exitPoolAddress = exitPoolAddressFor(cfg);

  applyPayVerifications(entries, dayExit, exitPoolAddress, verifyMs);
...
function replayPoolTimeline(poolId, purchaseTxs, exitPoolTxs, wallNowMs) {
  const cfg = poolConfig(poolId);
  const stateMap = new Map();
  const matchDays = [];
  const sortedPurchase = sortPoolTxs(purchaseTxs);

  if (!sortedPurchase.length) {
    const entries = [];
    return {
      entries,
      ...
      matchedCreditTrx: 0,
      matchDays,
      exitPoolAddress: exitPoolAddressFor(cfg),
      purchaseAddress: cfg.purchaseAddress,
    };
  }

  const firstTs = Math.min(...sortedPurchase.map((t) => t.blockTimestamp || wallNowMs));
  const firstMatchDay = utcDayMs(firstTs + ENTRY_PERIOD_DAYS * MS_DAY);
  const endDay = utcDayMs(wallNowMs);

  for (let dayMs = firstMatchDay; dayMs < endDay; dayMs += MS_DAY) {
  ...
  const finalPurchase = filterByCheckpoint(sortedPurchase, wallNowMs);
  let entries = mergeEntryStates(buildEntries(poolId, finalPurchase), stateMap);
  entries = applyLifecycle(entries);
  ...
  const payAssignments = split?.payAssignments || [];
...
function runPoolCycle(input) {
  ...
  const replay = replayPoolTimeline(poolId, buyTxs, exitPoolTxs, nowMs);
